...
export default function Authent() {
    ...
    function handleSubmit(event : any) {
        ...
        if (isNewAccount){
            ...
            else if (!RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})").test(password)){
                setErrorMessage("Password must contain at least 8 characters, one uppercase, one lowercase, one number and one special case character");
                setError(true);
            }
            else{
                setError(false);
        ...
        // ...
        // after successful account creation, redirect to account setup page
    ...
    return (
        ...
        <div>
            {error ? authError(errorMessage) : null}
        </div>
